<template>
  <main class="m-2">
    <div class="container col-lg-6 bg-white rounded">
      <h1 class="text-center text-danger fs-4 fs-lg-2 mt-2 pt-2 ">Créer un nouvel article</h1>
      <div class="col-lg-9 m-auto mt-3 mb-5">
        <form id="addPost" class="form">
          ...
          <div class="row justify-content-center align-items-center mt-md-5 text-center">
            ...
            <input @change="changeImage" class="file mt-3" type='file' name="image" accept=".png, .jpg, .jpeg, .webp, .gif, .mp4" />
          ...
          <div class="form-group  mb-2 d-grid gap-2 py-2 text-center "> 
            <button  @click.prevent="addPost()"  class="btn btn-danger fs-5 fw-bold" type="button">Envoyer</button>
          </div>
         </div>
        </form>
      </div>
  ...
  methods:{
    ...
    const formData = new FormData();
      ...
      if(this.title && this.content){
        axios.post('http://localhost:3005/api/post', formData)
        .then(() =>{
          this.$router.push("/articles")
        })
        .catch(error =>{error})
        ...
        alert('Veuillez remplir les champs nécessaires à la création de l\'article');
      }
    },
  }
}
